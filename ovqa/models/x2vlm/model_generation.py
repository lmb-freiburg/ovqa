...
from ovqa.models.x2vlm.xbert import BertLMHeadModel
from ovqa.models.x2vlm.xvlm import XVLMBase, XVLMPlusBase, load_pretrained
...
class XVLMForMLMCaptioning(XVLMBase):
    ...
    def __init__(self, config):
        ...
        self.crit_mask_lm_smoothed = LabelSmoothingLoss(
            ...
            ignore_index=self.tokenizer.cls_token_id,
            reduction="none",
        )
    ...
    def generate(
        ...
        bsz = image.size(0)
        input_ids = (
            torch.tensor(self.prompt_ids, dtype=torch.long, device=image.device)
            .view(1, -1)
            .expand(bsz, -1)
        )
        ...
        position_ids = (
            torch.tensor(range(length), dtype=torch.long, device=image.device)
            .view(1, -1)
            .expand(bsz, -1)
        )
        attention_mask = (
            torch.tril(torch.ones((length, length), dtype=torch.long, device=image.device))
            .view(1, length, length)
            .expand(bsz, length, length)
        )
...
class XVLMForVQA(XVLMBase):
    ...
    def __init__(self, config):
        ...
        assert config["num_dec_layers"] in [
            self.num_cross_layers,
            self.num_cross_layers // 2,
        ], "initialization not implemented"

        if config["text_encoder"] == "data/roberta-base":
            from ovqa.models.x2vlm.xroberta import RobertaConfig

            config_dec = RobertaConfig.from_json_file(
                os.path.join(config["text_encoder"], "config.json")
            )
        else:
        ...
        if config["text_encoder"] == "data/roberta-base":
            from ovqa.models.x2vlm.xroberta import RobertaForCausalLM
        ...
        if config.get("large_lr_for_dec", False):
            self.init_params = [
                "text_decoder." + n for n, _ in self.text_decoder.named_parameters()
            ]
        else:
            if self.dec_encoder_width != self.cross_encoder_width:
                self.init_params = [
                    "text_decoder." + n
                    for n, _ in self.text_decoder.named_parameters()
            ...
            for key in list(state_dict.keys()):
                ...
                if "text_encoder." in key:
                    if "layer." in key:
                        ...
                        if layer_num < self.num_text_layers:
                        ...
                        elif (config["num_dec_layers"] == self.num_cross_layers // 2) and (
                            layer_num % 2 == 0
                        ):  # 这里加了
                            del state_dict[key]
                            continue
                            ...
                            ("crossattention.self.key" in key)
                            or ("crossattention.self.value" in key)
                        ):
                            del state_dict[key]
                            continue
                            ...
                            if (
                                config["num_dec_layers"] == self.num_cross_layers // 2
                            ):  # 这里加上, 上面首先删掉了偶数层。
    ...
    def forward(self, image, quesiton, answer=None, k=None, weights=None, train=True):
        ...
        if train:
            ...
            answer_targets = answer.input_ids.masked_fill(
                answer.input_ids == self.pad_token_id, -100
            )
...
class XVLMPlusForVQA(XVLMPlusBase):
    ...
    def tie_enc_and_dec_wordemb(self):
        # tie word embeddings
        self.text_decoder.bert.embeddings.word_embeddings.weight = (
            self.text_encoder.embeddings.word_embeddings.weight
        )
        ...
        # tie
        self.text_decoder.cls.predictions.decoder.weight = (
            self.text_decoder.bert.embeddings.word_embeddings.weight
        )
    ...
    def forward(self, image, quesiton, answer=None, k=None, weights=None, train=True):
        ...
        if train:
            ...
            question_output = self.get_cross_embeds(
                image_embeds,
                image_atts,
                text_ids=quesiton.input_ids,
            ...
            topk_ids, topk_probs = self.rank_answer(
                question_output, quesiton.attention_mask, answer.input_ids, answer.attention_mask, k
            )
            return topk_ids, topk_probs
