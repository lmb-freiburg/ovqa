...
from ovqa.models.x2vlm.xvlm import XVLMBase, XVLMPlusBase, build_mlp
...
class XVLMForClassification(XVLMBase):
    def __init__(self, config):
        ...
        feature_dim = (
            self.vision_width if config.get("task_name") == "imagenet" else self.text_width
        )
    ...
    def forward(self, image, text_ids, text_atts, targets=None, train=True):
        if image is None:
            ...
            output_cls = self.get_cross_embeds(
                image_embeds, image_atts, text_ids=text_ids, text_atts=text_atts
            )[:, 0, :]
...
class XVLMForNLVR(XVLMBase):
    ...
    def forward(self, image, text_ids, text_atts, targets, train=True):
        ...
        output_cls_image1 = self.get_cross_embeds(
            image0_embeds,
        ...
        output_cls_image2 = self.get_cross_embeds(
            image1_embeds,
            image_atts[image0_embeds.size(0) :],
            text_ids=text_ids,
            text_atts=text_atts,
