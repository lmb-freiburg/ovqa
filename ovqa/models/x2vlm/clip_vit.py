...
class CLIPEncoderLayer(nn.Module):
    def __init__(
        self, hidden_size, hidden_act, num_attention_heads, attention_dropout, intermediate_size
    ):
        super().__init__()
...
class CLIPEncoder(nn.Module):
    ...
    def forward(
        ...
        if do_gather and (image_atts is not None):
            full_atts = torch.ones(inputs_embeds.shape[:2], dtype=inputs_embeds.dtype).to(
...
class CLIPVisionTransformer(nn.Module):
    def __init__(
        ...
        self.encoder = CLIPEncoder(
            ...
            intermediate_size,
            num_hidden_layers,
            local_attn_depth=local_attn_depth,
        )
