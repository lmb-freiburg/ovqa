        ...
        for query, _key, _value, part_length in zip(
            ...
            outputs.append(
                self.multi_head_attention(
                    ...
                    key,
                    value,
                    attention_mask[:, :, sum_length - part_length : sum_length, :sum_length],
                    rel_pos=None,
                )[0]
            )
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num, drop_path_rate=0.0):
        ...
        if self.has_cross_attention:
            ...
            self.crossattention = BertAttention(
                config, is_cross_attention=True, drop_path_rate=drop_path_rate
            )
    ...
    def forward(
        ...
        if self.has_cross_attention and (
            encoder_hidden_states is not None
        ):  # update: to utilize text data
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        print(
            f"### text_drop_path_rate: {text_drop_path_rate}, cross_drop_path_rate: {cross_drop_path_rate}",
            flush=True,
        )
        ...
        self.layer = nn.ModuleList(
            [BertLayer(config, i, dpr[i]) for i in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        for i in range(start_layer, output_layer):
            if history_states is not None:
                assert isinstance(history_states, list) and len(history_states) == (
                    output_layer - start_layer + 1
                )
            ...
            if output_hidden_states:
                all_hidden_states = all_hidden_states + (
                    hidden_states,
                )  # embedding output is the first one
