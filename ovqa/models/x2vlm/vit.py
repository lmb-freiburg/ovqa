...
class Block(nn.Module):
    ...
    def forward(self, x, register_hook=False, image_atts=None):
        x = x + self.drop_path(
            self.attn(self.norm1(x), register_hook=register_hook, image_atts=image_atts)
        )
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.depth = depth
        self.local_attn_depth = (
            local_attn_depth  # do local attn from index=(depth - local_attn_depth)
        )
