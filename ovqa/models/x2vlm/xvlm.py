...
from ovqa.models.x2vlm.xbert import BertConfig, BertForMaskedLM, BertModel
from ovqa.models.x2vlm.xroberta import RobertaForMaskedLM, RobertaModel, RobertaConfig
from packg.iotools.jsonext import load_json as read_json
...
def load_params_choose_layers(prefix: str, state_dict: dict, mapper: dict, do_expand=False):
    ...
    assert len(set(mapper.values())) == len(
        mapper
    ), f"{set(mapper.values())} != {len(mapper)}"  # no overlap
...
def get_bert_config(encoder_rpath, num_hidden_layers=12, cross_start_at=12):
    ...
    if "roberta" in encoder_rpath:
        try:
            ...
            print(f"Loading config {encoder_rpath} from pretrained")
            config = RobertaConfig.from_pretrained(encoder_rpath)
    else:
        try:
            config = BertConfig.from_json_file(os.path.join(encoder_rpath, "config.json"))
        except:
            print(f"Loading config {encoder_rpath} from pretrained")
            config = BertConfig.from_pretrained(encoder_rpath)
...
def build_vision_encoder(config, load_params=False):
    ...
    if config.get("use_clip_vit", False):  # good performance, but only base model available
        from ovqa.models.x2vlm.clip_vit import CLIPVisionTransformer, interpolate_pos_embed
        ...
        if load_params:
            ...
            pos_embed_reshaped = interpolate_pos_embed(
                state_dict["pos_embed.weight"].unsqueeze(dim=0),
                num_patches=num_patches,
                num_extra_tokens=1,
            )
            state_dict["pos_embed.weight"] = pos_embed_reshaped.squeeze(dim=0)

            assert vision_config["num_hidden_layers"] in [
                6,
                12,
            ], "param initialization not implemented"
        ...
        from ovqa.models.x2vlm.swin_transformer import SwinTransformer
        ...
        vision_encoder = SwinTransformer(
            ...
            add_cls=config.get("swin_add_cls", True),
        )

        if load_params:
            from ovqa.models.x2vlm.swin_transformer import load_pretrained_swin
        ...
        if "base" in config["vision_config"]:
            from ovqa.models.x2vlm.beit2 import beit_base_patch16 as beit_model
        elif "large" in config["vision_config"]:
            from ovqa.models.x2vlm.beit2 import beit_large_patch16 as beit_model
        else:
            raise ValueError

        vision_encoder = beit_model(
            ...
            init_values=0.1,
            qkv_bias=True,
            local_attn_depth=config.get("local_attn_depth", -1),
            vision_num_hidden_layers=config.get("vision_num_hidden_layers", -1),
        )

        if load_params:
            from ovqa.models.x2vlm.beit2 import load_pretrained_beit2
...
def build_text_encoder(
    config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None
):
    if config_text is None:
        config_text = get_bert_config(
            config["text_encoder"],
    ...
    config_text.hidden_dropout_prob = config.get(
        "dropout", config_text.hidden_dropout_prob
    )  # changable dropout rate
    ...
    if load_text_params:
        print(
            "### Initializing text encoder from ",
            os.path.join(config["text_encoder"], "pytorch_model.bin"),
        )
        state_dict = torch.load(
            os.path.join(config["text_encoder"], "pytorch_model.bin"), map_location="cpu"
        )
        if "model" in state_dict.keys():
            state_dict = state_dict["model"]

        prefix = (
            "roberta.encoder.layer" if "roberta" in config["text_encoder"] else "bert.encoder.layer"
        )
        if not use_mlm_loss:
            state_dict = {
                k.replace("roberta.", "").replace("bert.", ""): v for k, v in state_dict.items()
            }
...
def load_pretrained(model, ckpt_rpath, config, is_eval=False, load_text=False):
    ...
    if config.get("use_clip_vit", False):
        from ovqa.models.x2vlm.clip_vit import interpolate_pos_embed

        del state_dict["vision_encoder.position_ids"]
        num_patches = (config["image_res"] // config["patch_size"]) ** 2
        pos_embed_reshaped = interpolate_pos_embed(
            state_dict["vision_encoder.pos_embed.weight"].unsqueeze(dim=0),
            num_patches=num_patches,
            num_extra_tokens=1,
        )
        ...
        from ovqa.models.x2vlm.beit2 import interpolate_pos_embed
...
class XVLMBase(nn.Module):
    def __init__(
        ...
        self.text_encoder, missing_keys = self.build_text_encoder(
            config,
        ...
        self.cross_encoder, missing_keys = self.build_cross_encoder(
            config, self.text_encoder.config, load_cross_params=load_cross_params
        )
        ...
        if self.video_encoding != "":
            ...
            if self.add_frame_pos:
                self.absolute_frame_pos_embed = nn.Parameter(
                    torch.zeros(1, self.frame_len, 1, self.vision_width)
                )
                trunc_normal_(self.absolute_frame_pos_embed, std=0.02)
        ...
        if self.use_contrastive_loss:
            ...
            self.update_init_params(
                ["vision_proj." + n for n, _ in self.vision_proj.named_parameters()]
            )
            self.update_init_params(
                ["text_proj." + n for n, _ in self.text_proj.named_parameters()]
            )
        ...
        if self.use_bbox_loss:
            ...
            self.update_init_params(
                ["bbox_head." + n for n, _ in self.bbox_head.named_parameters()]
            )
    ...
    def build_text_encoder(
        self, config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None
    ):
        """
        in XVLMBase, text_encoder includes cross encoder parts
        """
        text_encoder, missing_keys = build_text_encoder(
            config,
            vision_width,
            load_text_params=load_text_params,
            use_mlm_loss=use_mlm_loss,
        ...
        if is_domain_pretrain:
            ...
            if config.get("init_timesformer", False):
                ...
                for from_key, to_key in map_dict.items():
                    for key in list(state_dict.keys()):
                        if to_key in key:
                            state_dict[key.replace(to_key, from_key)] = copy.deepcopy(
                                state_dict[key]
                            )
        else:
            state_dict = load_pretrained(self, ckpt_rpath, config, is_eval=is_eval, load_text=True)

        if hasattr(self, "absolute_frame_pos_embed") and (
            "absolute_frame_pos_embed" in state_dict.keys()
        ):
            ...
            if pretrained.shape != self.absolute_frame_pos_embed.shape:
                ...
                self.absolute_frame_pos_embed.data[:, :frame_len, :, :] = pretrained.data[
                    :, :frame_len, :, :
                ]
                print(
    ...
    def _encode_frames(self, frames, output_hidden_states=None, output_attentions=None):
        ...
        outputs = self.vision_encoder(
            frames, output_hidden_states=output_hidden_states, output_attentions=output_attentions
        )
    ...
    def _encode_video_pooling(self, frames, output_hidden_states=None, output_attentions=None):
        bsz = frames.shape[0]
        frame_embeds, outputs = self._encode_frames(
            frames, output_hidden_states=output_hidden_states, output_attentions=output_attentions
        )
    ...
    def get_frame_embeds(self, frame, output_hidden_states=None, output_attentions=None):
        ...
        if self.video_encoding == "avgpool":
            frame_embeds, outputs = self._encode_video_pooling(
                frame,
    ...
    def get_image_embeds(
        ...
        if idx_to_group_img is None:
            ...
            if image_atts is None:
                ...
                image_embeds_fullatts = torch.gather(
                    image_embeds_fullatts,
                    dim=0,
                    index=idx_to_group_img.view(-1, 1, 1).expand(
                        -1, image_embeds_fullatts.shape[1], image_embeds_fullatts.shape[2]
                    ),
                )  # expend to bsz

                image_atts = torch.ones(image_embeds_fullatts.size()[:-1], dtype=torch.long).to(
                    image.device
                )
                ...
                image_embeds, image_embeds_fullatts = self.vision_encoder(
                    image, idx_to_group_img=idx_to_group_img, image_atts=image_atts
                )
    ...
    def get_text_embeds(
        self, text_ids, text_atts, output_hidden_states=None, output_attentions=None
    ):
        assert output_hidden_states == output_attentions

        encoder = (
            self.text_encoder.bert if hasattr(self.text_encoder, "bert") else self.text_encoder
        )
    ...
    def get_features(self, image_embeds=None, text_embeds=None):
        if image_embeds is None:
            ...
            return F.normalize(self.vision_proj(image_embeds[:, 0, :]), dim=-1), F.normalize(
    ...
    def get_contrastive_loss(self, image_feat, text_feat, idx=None):
        ...
        image_feat_all = allgather(
            image_feat, torch.distributed.get_rank(), torch.distributed.get_world_size()
        )
        text_feat_all = allgather(
            text_feat, torch.distributed.get_rank(), torch.distributed.get_world_size()
        )
        ...
        if idx is None:
            ...
            idx_all = allgather(
                idx, torch.distributed.get_rank(), torch.distributed.get_world_size()
            )
    ...
    def get_matching_loss(
        self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None
    ):
        """
        ...
        cross_pos = self.get_cross_embeds(
            image_embeds, image_atts, text_embeds=text_embeds, text_atts=text_atts
        )[:, 0, :]
        cross_neg = self.get_cross_embeds(
            image_embeds_all, image_atts_all, text_embeds=text_embeds_all, text_atts=text_atts_all
        )[:, 0, :]
    ...
    def get_mlm_loss(
        self, text_ids_masked, text_atts, image_embeds, image_atts, masked_pos, masked_ids
    ):
    ...
    def get_bbox_loss(self, output_coord, target_bbox_ex, is_image=None, target_bbox_map_ids=None):
        ...
        if (boxes1[:, 2:] < boxes1[:, :2]).any() or (boxes2[:, 2:] < boxes2[:, :2]).any():
            ...
            print(
                "### (boxes1[:, 2:] < boxes1[:, :2]).any() or (boxes2[:, 2:] < boxes2[:, :2]).any()"
            )
...
class XVLMPlusBase(XVLMBase):
    ...
    def __init__(
        ...
        if use_mlm_loss:
            from ovqa.models.x2vlm.xbert import BertOnlyMLMHead

            self.mlm_head = BertOnlyMLMHead(self.text_encoder.config)
            self.update_init_params(
                ["mlm_head." + n for (n, _) in self.mlm_head.named_parameters()]
            )
    ...
    def build_cross_encoder(self, config, config_text, load_cross_params=False):
        config_cross = get_bert_config(
            config["cross_encoder"],
            num_hidden_layers=config["cross_num_hidden_layers"],
            cross_start_at=0,
        )
        ...
        cross_encoder = BertModel(
            config=config_cross, add_pooling_layer=False, add_embeddings_layer=False
        )
        ...
        if load_cross_params:
            print(
                "### Initializing cross encoder from ",
                os.path.join(config["cross_encoder"], "pytorch_model.bin"),
            )
            state_dict = torch.load(
                os.path.join(config["cross_encoder"], "pytorch_model.bin"), map_location="cpu"
            )
            if "model" in state_dict.keys():
                state_dict = state_dict["model"]
                ...
                if k.startswith("text_encoder."):
                    if "layer" in k:
                        ...
                        if layer_num < num_text_layers:
                            ...
                            new_k = new_k.replace(
                                f"layer.{layer_num}.", f"layer.{layer_num-num_text_layers}."
                            )
                            state_dict[new_k] = state_dict[k]
                            del state_dict[k]
                    else:
    ...
    def load_pretrained_text(self, text_ckpt_rpath, config, is_eval=False):
        ...
        for k in list(text_enc_state_dict.keys()):
            if k.startswith("roberta.") or k.startswith("bert."):
                encoder_key = (
                    "text_encoder." + k.replace("roberta.", "").replace("bert.", "").strip()
                )
    ...
    def load_pretrained(self, xvlm_ckpt_rpath, config, is_eval=False, text_ckpt_rpath=""):
        ...
        if config.get("is_xvlm_ckpt", False):
            ...
            state_dict = load_pretrained(
                self, xvlm_ckpt_rpath, config, is_eval=is_eval, load_text=True
            )

        if config.get("replace_text_encoder", False):
            text_enc_state_dict = self.load_pretrained_text(
                text_ckpt_rpath, config, is_eval=is_eval
            )
    ...
    def get_cross_embeds(
        ...
        if text_embeds is None:
            ...
            ), "please manually split get_text_embeds and get_cross_embeds"
    ...
    def get_mlm_loss(
        ...
        sequence_output = self.get_cross_embeds(
            image_embeds, image_atts, text_ids=text_ids_masked, text_atts=text_atts
        )
