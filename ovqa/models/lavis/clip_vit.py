...
from ovqa.models.lavis.eva_vit import convert_weights_to_fp16
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        if use_grad_checkpointing:
            raise NotImplementedError("Install fairscale and reenable if needed")
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.resblocks = nn.Sequential(
            *[
                ResidualAttentionBlock(width, heads, attn_mask, use_grad_checkpointing and i > 12)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(
            width, layers - 1, heads, use_grad_checkpointing=use_grad_checkpointing
        )
...
def create_clip_vit_L(img_size=224, use_checkpoint=False, precision="fp16"):
    ...
    logging.info(f"Done loading vision encoder ckpt {type(model).__name__} from {url}.")
