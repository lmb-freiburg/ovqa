...
from ovqa.metrics.lerc.allennlp.common.params import Params, remove_keys_from_params
...
from ovqa.metrics.lerc.allennlp.module import Module
from ovqa.metrics.lerc.allennlp.regularizer_applicator import RegularizerApplicator
...
class Model(Module, Registrable):
    ...
    def forward_on_instances(self, instances: List[Instance]) -> List[Dict[str, numpy.ndarray]]:
        ...
        with torch.no_grad():
            ...
            model_input = nnutil.move_to_device(dataset.as_tensor_dict(), cuda_device)
    ...
    def _get_prediction_device(self) -> int:
        ...
        """
        devices = {nnutil.get_device_of(param) for param in self.parameters()}
    ...
    def _load(
        ...
        model_state = nnutil.read_state_dict(weights_file, cuda_device=cuda_device)
