...
from ovqa.metrics.lerc.allennlp import nnutil
from ovqa.metrics.lerc.allennlp.archival import Archive, load_archive
from ovqa.metrics.lerc.allennlp.batch import Batch
from ovqa.metrics.lerc.allennlp.common.registrable import Registrable
from ovqa.metrics.lerc.allennlp.common.util import JsonDict, sanitize
...
class Predictor(Registrable):
    ...
    def get_gradients(self, instances: List[Instance]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        dataset_tensor_dict = nnutil.move_to_device(dataset.as_tensor_dict(), self.cuda_device)
    ...
    def get_interpretable_layer(self) -> torch.nn.Module:
        ...
        """
        try:
            return nnutil.find_embedding_layer(self._model)
        except RuntimeError:
            raise RuntimeError(
    ...
    def get_interpretable_text_field_embedder(self) -> torch.nn.Module:
        ...
        """
        try:
            return nnutil.find_text_field_embedder(self._model)
        except RuntimeError:
            raise RuntimeError(
                "If the model does not use `TextFieldEmbedder`, please override "
    ...
    def _register_embedding_gradient_hooks(self, embedding_gradients):
        ...
        def hook_layers(module, grad_in, grad_out):
            ...
            if self._token_offsets:
                ...
                span_grads, span_mask = nnutil.batched_span_select(grads.contiguous(), offsets)
        ...
        def get_token_offsets(module, inputs, outputs):
            offsets = nnutil.get_token_offsets_from_text_field_inputs(inputs)
    ...
    def from_path(
        ...
        frozen: bool = True,
        import_plugins: bool = False,  # True,
