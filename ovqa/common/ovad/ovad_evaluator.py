...
This API was based from the evaluation API:
...
class AttEvaluator(object):
    def __init__(
        ...
        self.attribute_parent_type = attr_parent_type if attr_parent_type is not None else {}
        ...
        self.attribute_seen_unseen = att_seen_unseen if att_seen_unseen is not None else {}
    ...
    def register_gt_pred_vectors(self, gt, pred, instance_info):
        assert (
            pred.shape == gt.shape
        ), "GT ({}) and prediction ({}) have to be the same shape".format(gt.shape, pred.shape)
    ...
    def evaluate(self, pred, gt_label, threshold_type="threshold"):
        ...
        groups_overall = {k: GroupClassMetric(metric_type="overall") for k in self.all_groups}
        groups_per_class = {k: GroupClassMetric(metric_type="per-class") for k in self.all_groups}
    ...
    def plot_pr_curves_types(self, save_dir, category):
        if category in self.attribute_parent_type.keys():
            for subtype in self.attribute_parent_type[category]:
                ...
                if max_r > 0 or max_p > 0:
                    path_plot = os.path.join(save_dir, "pr_curves_{}.png".format(subtype))
                    # print("Saving pr plot {}".format(path_plot))
                    plt.savefig(path_plot)
                plt.close("all")

    def print_evaluation(self, pred=None, gt_label=None, output_file="", percentage=True):
        # Compute scores.
        if pred is not None and gt_label is not None:
            ...
            self._clear_cache()
            scores_overall, scores_per_class = self.evaluate(pred.copy(), gt_label.copy())
        ...
        if output_file != "":
            ...
            self.logger.info("Saving results in file: " + output_file)
            self.logger.info("Dataset: " + output_file)
            ...
            with open(output_file, "a") as f:
                for category in CATEGORIES:
                    ...
                    if category in self.attribute_parent_type.keys():
                        self.plot_pr_curves_types(os.path.dirname(output_file), category)
            ...
            with open(output_file, "a") as f:
                ...
                for i_class in range(self.n_class):
                    ...
                    scores_cls_top_k = self.get_score_class(i_class, threshold_type="topk")
                    f.write(
                        "| {:<50}\t| {:.3f}\t| {:.4f}\t| {:.4f}\t| {:<6}| {:<6}| {:.4f}\t|\n".format(
                            att[:48],
                            scores_cls_thres.ap,
...
def print_metric_table(att_evaluator, results, file_print=""):
    all_metrics = [
        "PCrand_P",
        "PCt{thr}_ap",
        # "PCt{thr}_R",
...
if __name__ == "__main__":
    ...
    for ann in gt_ovad_api.anns.values():
        gt_object_classes.append(np.asarray(ann["category_id"]))
    ...
    freq_att = (ground_truth_labels == 1).sum(0)
    ...
    overall_metrics["random"] = print_metric_table(evaluator, results)
    overall_metrics["random"]["#instances"] = len(random_predictions)

    results_table = {key: list(metrics.values()) for key, metrics in overall_metrics.items()}
    results_table["Type"] = [
        att_type.replace("PCt0.5_ap/", "") for att_type in overall_metrics["random"].keys()
    ]
