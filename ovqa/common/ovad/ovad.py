...
API for accessing OVAD Dataset: 
OVAD API is a Python API that assists in loading, parsing and visualizing
the annotations in OVAD. 
This API was based on LVIS API:
https://github.com/lvis-dataset/lvis-api/blob/master/lvis/lvis.py
"""
...
class OVAD:
    ...
    def _create_index(self):
        ...
        if "attributes" in self.dataset.keys():
    ...
    def load_atts(self, ids):
        """Load attributes with the specified ids. If ids=None load all
        attributes.
        Args:
            ids (int array): integer array of attribute ids
        Returns:
            atts (dict array) : loaded attribute dicts
        """
        return self._load_helper(self.atts, ids)
